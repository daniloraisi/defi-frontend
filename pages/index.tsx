...
export default function Home() {
  ...
  const [addEther, setAddEther] = useState(zero.toNumber());
  ...
  const _getAmountOfTokensReceivedFromSwap = async (_swapAmount: string) => {
    try {
      ...
      if (!_swapAmountWEI.eq(zero)) {
        const [provider, _] = await getProviderOrSigner();
   ...
   * otherwise
   */
  const getProviderOrSigner = async (): Promise<[providers.Provider, Signer]> => {
    // Connect to Metamask
            ...
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                <input
                  ...
                  onChange={(e) => setAddEther(Number(e.target.value || 0))}
              ...
              <div>
                <input
                  ...
                  placeholder="Amount of Ether"
                  onChange={async (e) => {
                    setAddEther(Number(e.target.value || "0"));
